-- Fold settled invoices into the shift totals.
--
-- Recreated from 20260807110000_roles_and_cashup_amend.sql with one change:
-- sales are now orders PLUS invoices settled in the shift. Everything else —
-- petty cash, refunds, stock variance, the expected-cash arithmetic — is
-- unchanged.
--
...
begin
  ...
  select
    coalesce(sum(i.total_amount) filter (where lower(coalesce(i.payment_method,'')) = 'cash'), 0),
    coalesce(sum(i.total_amount) filter (where lower(coalesce(i.payment_method,'')) = 'card'), 0),
    coalesce(sum(i.total_amount) filter (where lower(coalesce(i.payment_method,'')) = 'eft'), 0),
    coalesce(sum(i.total_amount) filter (
      where lower(coalesce(i.payment_method,'')) not in ('cash','card','eft')), 0),
